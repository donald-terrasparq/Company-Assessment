...
/**
 * Settings → Scoring (admin-only): the global weight controls that used to
 * sit at the bottom of the Signals tab — fit weights, recency curve, source
 * confidence, tier thresholds, category boost, and the caveat-cap toggle.
 * Same live impact preview: scoring is pure, so tiers recompute instantly.
 */
...
export function GlobalControlsEditor({
  initialWeights,
  companies,
}: {
  ...
  const preview = useMemo(() => {
    ...
    for (const c of companies) {
      ...
      if (next.tier !== c.tier) {
        changed++;
        if ((TIER_ORDER[next.tier] ?? 0) - (TIER_ORDER[c.tier] ?? 0) > 0) promoted++;
        else demoted++;
      }
    }
    return { promoted, demoted, changed, tierCounts };
  }, [companies, weights]);
          ...
          <div>
            ...
            <label className="mt-4 flex cursor-pointer items-start gap-2 rounded-[10px] border border-line-2 bg-[#FBFCFD] px-3 py-2.5">
              <input
                ...
                onChange={(e) => setWeights((w) => ({ ...w, caveat_caps: e.target.checked }))}
                className="mt-0.5"
              />
        ...
        <section className="rounded-card border border-line bg-card p-5 shadow-card">
          ...
          <div className="flex flex-col gap-2">
            ...
            <p className="text-[10.5px] leading-[1.4] text-muted">
              Applying re-scores stored signals — no API calls, no cost. Signal strengths
              are tuned on the Signals tab; these weights save together with them.
            </p>
          </div>
        </section>
      </div>
