...
CREATE TABLE users (
  ...
  email          TEXT UNIQUE,
  first_name     TEXT,                -- 0009: from the invite
  last_name      TEXT,
...
-- one-time invite codes (Settings → Users → Invite user); 30-day expiry
...
CREATE TABLE companies (
  ...
  contact_name_hints  TEXT[] NOT NULL DEFAULT '{}',
  contact_title_hints TEXT[] NOT NULL DEFAULT '{}',
  created_at   TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
-- ─────────────────────────── company profiles (0009) ───────────────────────────
-- who the SELLER is: re-brandable company facts, up to 4 products mapped onto
-- the four internal signal slots, and AI context steering research/emails
...
-- ─────────────────────────── user annotations (0015) ───────────────────────────
-- notes, per-user viewed state, manual contacts, contact search hints.
-- Keyed on companies (not company_results) so re-analysis never touches them.
